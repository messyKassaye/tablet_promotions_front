...
import {storeBank} from "../../state/action/AdminBankAction";
...
class AddNewBank extends Component {
    ...
    componentWillReceiveProps(nextProps, nextContext) {
        ...
        if (nextProps.response.status) {
          ...
          setTimeout(()=>{
              this.props.showMainDialog({'show':false,'page':null,'title':''})
          },2000)
        }
    }
    ...
    render() {
        ...
        const isEnabled = formData.bank_name.length > 0 && formData.logo_path.length > 0
        return (
...
export default connect(mapStateToProps, {storeBank, showMainDialog, storeBase64})
